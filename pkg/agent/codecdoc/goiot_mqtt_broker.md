...
> OnConnect 的 `GetSession()` / `GetMessage()` 为 nil。认证前还没有设备，不要用 `context.GetConfig`，用 `GetDeviceById(GetClientId()).GetConfig(...)`。clientId 就是设备 id。`device.GetConfig` 先读设备，值为空则回落产品配置。
