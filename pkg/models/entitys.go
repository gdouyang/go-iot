...
type User struct {
	...
	CreateTime DateTime `json:"createTime" orm:"column(create_time_);type(date)"`
}
...
type Product struct {
	...
	NetworkType string            `json:"networkType" orm:"column(type_);size(32);description(网络类型MQTT_BROKER)"`             // 网络类型MQTT_BROKER
	...
	Tag         map[string]string `json:"tag,omitempty" orm:"column(tag_);null;description(标签)"`                         // 标签
...
type Device struct {
	...
	Metaconfig map[string]string `json:"metaconfig,omitempty" orm:"column(meta_config_);null;description(配置属性)"`
	Tag        map[string]string `json:"tag,omitempty" orm:"column(tag_);null;description(标签)"`
